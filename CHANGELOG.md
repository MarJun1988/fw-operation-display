...
- Zusammengeschriebene Straßennamen wie `AmRosenhag` werden für die Kartensuche getrennt.
- Die Karte verwendet die aufbereitete Einsatzadresse für die Koordinatensuche.
- Die Prisma-Konfiguration lädt `backend/.env`, damit `DATABASE_URL` bei CLI-Befehlen verfügbar ist.
- Das Seed-Skript wird mit Node.js gestartet und enthält den Versionseintrag `v.2.1.5`.

### 🔧 Changed

- Herdr-Startskript für Backend, Frontend und VitePress ergänzt.
...
- Adressauflösung verbessert, mit Ort-Ortsteil
...
- Verbidnugsproblme im zusammenhang mit dem Websocket Protkoll behoben
