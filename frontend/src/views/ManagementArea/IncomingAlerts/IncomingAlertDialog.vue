...
const saveIncomingAlert = () => {
  if (formIncomingAlert.value?.text?.trim()) {
    if (formIncomingAlert.value.id) {
      store.updateItem(formIncomingAlert.value)
    } else {
      store.createItem(formIncomingAlert.value)
    }
...
<template>
  ...
  >
    ...
    <div class="grid grid-cols-4 gap-7 mt-7">
      ...
      <DialogIncomingAlert :is-disabled="false" />
    </div>
