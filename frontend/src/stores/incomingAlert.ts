...
export const useIncomingAlertStore = defineStore('incomingAlertStore', () => {
  ...
  const getLastIncomingAlertAddress = computed(() => {
    if (pagedItems.value && pagedItems.value.length > 0 && pagedItems.value[0]) {
      ...
      console.log('getLastIncomingAlertAddress - Regex match: ', matches)

      if (matches && matches.length > 1 && matches[1]) {
        // Trennung Ort und Ortsteil
        const addressSplit = matches[1].split('-')

        console.log('getLastIncomingAlertAddress - addressSplit: ', addressSplit)
        ...
        if (addressSplit && addressSplit.length > 0 && addressSplit[1]) {
          addressByCaps = addressSplit.slice(1).join('-')
            .replace(/([\p{Ll}\d])(\p{Lu})/gu, '$1 $2')
            .trim()
            .replace(/\bStr\./gi, 'Straße')
        }

        console.log('getLastIncomingAlertAddress - addressByCaps ', addressByCaps)
        ...
        console.log('getLastIncomingAlertAddress - result ', result)
        return result
      }
    }
    return {
