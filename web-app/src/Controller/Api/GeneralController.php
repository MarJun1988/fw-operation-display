...
{
    private const LOGGER_ALL_ENTRIES = "Federal States";
    private const LOGGER_ONE_ENTRY = "Federal State";
    ...
    public function readAll(SerializerInterface $serializer, Request $request): Response
    {
        ...
        $count = $cache->getItem('stats.general_count');
        $count->expiresAfter(1);
        $filterCount = $cache->getItem('stats.general_count');
        $filterCount->expiresAfter(1);
        $items = $cache->getItem('stats.general_items');
        $items->expiresAfter(1);
        $serialize = $cache->getItem('stats.general_serialize');
        ...
        if (!$count->isHit() ||
            ...
            $itemsDatabase = $this->generalRepository->findByDataTableService($request, $this->dataTableService);
    ...
    public function new(Request $request): Response
    {
        ...
        $item = (new General())
            ->setReloadTimeAlert((int)$requestEntry['reloadTimeAlert'])
            ->setReloadTimeStyle((int)$requestEntry['reloadTimeStyle'])
            ->setReloadTimeGeneral((int)$requestEntry['reloadTimeGeneral'])
            ->setReloadTimeMessage((int)$requestEntry['reloadTimeMessage'])
            ->setShowReloadToast(isset($requestEntry['showReloadToast']) ? (bool)$requestEntry['showReloadToast'] : false)
            ->setDiveraUrl((string)$requestEntry['diveraUrl'])
    ...
    public function edit(Request $request): Response
    {
        ...
        if ($item) {
            $item
                ...
                ->setReloadTimeGeneral((int)$requestEntry['reloadTimeGeneral'])
                ->setReloadTimeMessage((int)$requestEntry['reloadTimeMessage'])
                ->setShowReloadToast((bool)$requestEntry['showReloadToast'])
                ->setDiveraUrl((string)$requestEntry['diveraUrl'])
                ->setComment(isset($requestEntry['comment']) ? (string)$requestEntry['comment'] : "")
