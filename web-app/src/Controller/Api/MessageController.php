...
{
    private const LOGGER_ALL_ENTRIES = "Messages";
    private const LOGGER_ONE_ENTRY = "Message";
    ...
    public function readAll(SerializerInterface $serializer, Request $request): Response
    {
        ...
        $count = $cache->getItem('stats.message_count');
        $count->expiresAfter(1);
        $filterCount = $cache->getItem('stats.message_count');
        $filterCount->expiresAfter(1);
        $items = $cache->getItem('stats.message_items');
        $items->expiresAfter(1);
        $serialize = $cache->getItem('stats.message_serialize');
        ...
        if (!$count->isHit() ||
            ...
            $itemsDatabase = $this->messageRepository->findByDataTableService($request, $this->dataTableService);
    ...
    public function getById(SerializerInterface $serializer, Request $request): Response
    {
        ...
        $items = $this->messageRepository->find($request->get('id'));
        // Objekte Konvertieren
    ...
    public function new(Request $request): Response
    {
        ...
        $item =(new Message())
            ->setHeadline((string)$requestEntry['headline'])
            ->setMessage((string)$requestEntry['message'])
    ...
    public function edit(Request $request): Response
    {
        ...
        $item = $this->messageRepository->find($request->get('id'));
        // Übermittelte Daten
        $requestEntry = $request->toArray();
