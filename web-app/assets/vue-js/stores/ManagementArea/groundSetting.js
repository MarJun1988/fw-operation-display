...
let nameOfTheStore = `GroundSetting`;
export const useGroundSettingStore = defineStore(`store${nameOfTheStore}`, () => {
    // State
    const dataById = ref({
        id: null,
        diveraUrl: '',
        reloadTimeGeneral: 60000,
        ...
        axios.get(url)
            .then(async (response) => {
                if (response && response.data && response.data.items) {
                    ...
                    await loadDataById(response.data.items[0].id);
                }
